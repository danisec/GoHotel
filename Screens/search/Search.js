...
import SearchComponent from '../../components/UI/organisms/Search';
...
export default function Search({navigation}) {
  ...
  const destructSearch = search?.hotels;

  const {favorites} = useSelector(state => state.favorites);

  const skeleton = () => {
    return [1, 2, 3, 4].map(item => <Skeleton key={item} />);
  };
  ...
  const handleFavorite = item => {
    favorites?.find(favorite => favorite.hotelId === item.hotelId)
  ...
  return (
    <SafeAreaView className="bg-white">
      <FlatList
        ...
        data={destructSearch}
        renderItem={status === 'loading' ? skeleton : renderItem}
      />
    </SafeAreaView>
  );
}
